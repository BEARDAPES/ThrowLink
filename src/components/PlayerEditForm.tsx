...
export function PlayerEditForm({ profile, player, offerConditions, onSave }: PlayerEditFormProps) {
  ...
  function pickHomeShop(shop: StoreOption) {
    setHomeShopId(shop.id)
    setHomeShopSelectedName(shop.display_name)
    setHomeShopText('')
    setHomeShopQuery('')
    setHomeShopResults([])
  }
                ...
                <div>
                  ...
                  {homeShopResults.length > 0 && (
                    <div className="mt-2 border border-brass/50 rounded-sm divide-y divide-brass/20">
                      {homeShopResults.map((shop) => (
                        <button
                          key={shop.id}
                          type="button"
                          onClick={() => pickHomeShop(shop)}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex gap-2">
                    <button type="button" onClick={() => setPricingType('per_event')} className={triButtonClass(pricingType === 'per_event')}>
                      イベント単位
                    </button>
                    <button type="button" onClick={() => setPricingType('per_hour')} className={triButtonClass(pricingType === 'per_hour')}>
                      時間単価
                    </button>
                  </div>
                </div>
